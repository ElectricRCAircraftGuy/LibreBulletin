...
THIS_FILENAME = "bulletin_find_and_replace.py"

print('LibreBulletin START OF "{}" IMPORTS.'.format(THIS_FILENAME))

# Internal modules from within this project
# 1. Dynamic imports
import config_master
from import_helper import extract_config_file_from_inputs
if (config_master.demo == True):
    config_master.bulletin_INPUTS = config_master.bulletin_INPUTS_default
    extract_config_file_from_inputs(config_master)
    from autogenerated import config 
else:
    extract_config_file_from_inputs(config_master)
    from MY_PERSONAL_WARD_INFO.autogenerated import config
# 2. Other internal imports
...
class Bulletin:
    ...
    def loadFields(self):
        ...
        # A. Sunday's date
        # Find the date of this coming Sunday. 
        # NB: If today is Sunday and it is before time_to_use_next_sunday_date O'Clock, then use today's date, but if
        # it is after this time, use next Sunday's date.
        self.this_sunday = date.getUpcomingSundayDateTime(config.time_to_use_next_sunday_date)
...
def main():
    print('\nLibreBulletin START OF "{}" OPERATIONS.'.format(THIS_FILENAME))
    if (config_master.demo == True):
